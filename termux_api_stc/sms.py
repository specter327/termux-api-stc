...
# Asynchronous API
# ==========
async def inbox_async(limit: Optional[int] = None, offset: Optional[int] = None,
          from_number: Optional[str] = None, msg_type: Optional[str] = None):
    """
    Wraps `termux-sms-list [-l limit] [-o offset] [-f from] [-t type]`.
    Devuelve una lista de dicts con los mensajes SMS.
    ...
    if msg_type is not None:
        args += ["-t", msg_type]
    return await run_async("termux-sms-list", args)


async def send_async(numbers: Union[str, List[str]], text: str, sim_slot: Optional[int] = None):
    """
    Wraps `termux-sms-send [-n numbers] [-s simslot] text`.
    :param numbers: numero de telefono, o lista de numeros
    """
    ...
    if sim_slot is not None:
        args += ["-s", str(sim_slot)]
    args.append(text)
    return await run_async("termux-sms-send", args, parse_json=False)
